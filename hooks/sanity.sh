#!/usr/bin/env bash
# WARNING: This hook can be slow. It runs the full tox-ansible sanity test suite.
set -euo pipefail
...
ENVS=$(python3 -m tox --ansible --conf tox-ansible.ini -l 2>/dev/null | grep "^sanity-" | paste -sd, - || true)
if [[ -z "$ENVS" ]]; then
    echo "No sanity test environments found."
    exit 0
fi
