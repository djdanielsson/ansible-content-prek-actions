...
Most hooks use [ansible-dev-tools==26.6.1](https://github.com/ansible/ansible-dev-tools)
...
The `galaxy-importer` hook uses `galaxy-importer==0.4.40` in its own isolated
...
Most hooks pin `ansible-dev-tools==26.6.1` for a known-good set of tool
...
Uses `galaxy-importer==0.4.40` in its own isolated environment (separate from
